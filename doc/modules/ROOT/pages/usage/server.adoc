...
nREPL server's behaviour via configuration files and environment
variables.

There are two configuration files:

* Global configuration file `.nrepl/config.edn`
* Local configuration file `nrepl-config.edn`
...
project). It's purpose it to set project-specific settings (e.g. a common port
you always want to use with a particular project). Any setting in `.nrepl-config.edn`
will take precedence over a setting in `.nrepl/config.edn`.
...
You can also use environment variable to configure nREPL's behaviour:
...
Environment variables have the highest precedence and will override
anything specified in the global or local server config files.
